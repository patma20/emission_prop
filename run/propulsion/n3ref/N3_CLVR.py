...
def viewer(prob, pt, file=sys.stdout):
    ...
    print(
        " %7.5f  %7.1f %7.3f %7.1f %7.1f %7.1f %7.3f  %7.5f  %7.3f  %7.5f  %7.5f" % summary_data,  #  %7.5f
        file=file,
        flush=True,
    )
...
class MPN3(pyc.MPCycle):
    ...
    def setup(self):
        ...
        # manually connect injector/extractor areas from CRZ to TOC
        ...
        # NOx Index Modeling
        indvars = self.add_subsystem("humidity", om.IndepVarComp(), promotes_outputs=["*"])
        indvars.add_output("H_TOC", alt_war)
        indvars.add_output("H_RTO", sls_war)
        indvars.add_output("H_CRZ", alt_war)
        indvars.add_output("H_SLS", sls_war)

        self.add_subsystem("TOC_EINOx", EINOx())
        self.add_subsystem("RTO_EINOx", EINOx())
        self.add_subsystem("CRZ_EINOx", EINOx())

        # self.connect("SLS.bld3.Fl_O:tot:P", "TOC_EINOx.P3_SLS")
        # self.connect("SLS.bld3.Fl_O:tot:T", "TOC_EINOx.T3_SLS")
        # self.connect("SLS.balance.FAR", "TOC_EINOx.FAR_SLS")
        self.connect("TOC.bld3.Fl_O:tot:P", "TOC_EINOx.P3_OD")
        self.connect("TOC.balance.FAR", "TOC_EINOx.FAR_OD")
        self.connect("H_TOC", "TOC_EINOx.h_OD")
        # self.connect("H_SLS", "TOC_EINOx.h_SLS")

        # self.connect("SLS.bld3.Fl_O:tot:P", "RTO_EINOx.P3_SLS")
        # self.connect("SLS.bld3.Fl_O:tot:T", "RTO_EINOx.T3_SLS")
        # self.connect("SLS.balance.FAR", "RTO_EINOx.FAR_SLS")
        self.connect("RTO.bld3.Fl_O:tot:P", "RTO_EINOx.P3_OD")
        self.connect("RTO.balance.FAR", "RTO_EINOx.FAR_OD")
        self.connect("H_RTO", "RTO_EINOx.h_OD")
        # self.connect("H_SLS", "RTO_EINOx.h_SLS")

        self.connect("SLS.bld3.Fl_O:tot:P", ("TOC_EINOx.P3_SLS", "RTO_EINOx.P3_SLS", "CRZ_EINOx.P3_SLS"))
        self.connect("SLS.bld3.Fl_O:tot:T", ("TOC_EINOx.T3_SLS", "RTO_EINOx.T3_SLS", "CRZ_EINOx.T3_SLS"))
        self.connect("SLS.balance.FAR", ("TOC_EINOx.FAR_SLS", "RTO_EINOx.FAR_SLS", "CRZ_EINOx.FAR_SLS"))
        self.connect("CRZ.bld3.Fl_O:tot:P", "CRZ_EINOx.P3_OD")
        self.connect("CRZ.balance.FAR", "CRZ_EINOx.FAR_OD")
